gp.exe -delete 6D797061636B616731 -deletedeps -verbose 
gp.exe -delete  A0000000035344 -deletedeps -verbose 
gp.exe -delete A0000000035350 -deletedeps -verbose 
gp.exe -delete 534B544558544C4942 -deletedeps -verbose 
gp.exe -delete A000000227011000 -deletedeps -verbose 

gp.exe -delete D276000005AA040360010410 -deletedeps -verbose 
gp.exe -delete D276000005AA0403010103 -deletedeps -verbose 

gp.exe -delete D156000132000401 -deletedeps -verbose 

gp.exe -install AlgTest_v1.6_supportOnly_jc222.cap -verbose 
